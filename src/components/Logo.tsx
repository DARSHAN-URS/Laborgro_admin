...
const Logo: React.FC<LogoProps> = ({ 
  ...
  const config = {
    blue: { main: "#3d7ab5", inner: "white", text: "text-[#3d7ab5]" },
    white: { main: "white", inner: "transparent", text: "text-white" },
    dark: { main: "#1a2533", inner: "white", text: "text-[#1a2533]" }
  }[variant];
    ...
    <div className={`flex items-center gap-2.5 group shrink-0 ${className}`}>
      <div className={`relative ${iconSize} flex items-center justify-center transition-transform duration-300 group-hover:scale-110`}>
        <svg viewBox="0 0 100 60" fill="none" className="w-full h-full drop-shadow-sm">
          <path d="M10 45C10 45 25 10 50 10C75 10 90 45 90 45L75 55C75 55 65 30 50 30C35 30 25 55 25 55L10 45Z" fill={config.inner} stroke={config.main} strokeWidth="6" strokeLinejoin="round" />
          <path d="M45 30L50 42L55 30" stroke={config.main} strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" />
